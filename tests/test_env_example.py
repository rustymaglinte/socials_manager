...
field names, plus the families that are built per brand -- because a crude
...
from app.domain.brand import account_id_env_var, all_brands
...
def _required_by_the_code() -> dict[str, str]:
    ...
    # are the families that multiply as brands are added.
    for brand in all_brands():
        ...
        # Ids and tokens, for every account a deployment could publish to: one
        # that brand.yaml enables, on a platform with a publisher adapter
        # (`PUBLISHABLE_PLATFORMS`). Anything else is refused before either
        # variable is looked up, so demanding it would be demanding a variable
        # the code cannot read.
        #
        # Keyed on `enabled_accounts`, not `publishable_accounts`. The id now
        # comes from the environment, so "publishable" depends on whose machine
        # runs this -- and a list of what a deployment needs cannot depend on
        # what the deployment already has.
        for account in brand.enabled_accounts:
            if account.platform not in PUBLISHABLE_PLATFORMS:
                continue
            id_variable = account_id_env_var(brand.slug, account.platform)
            if id_variable:
                required.setdefault(id_variable, "app/domain/brand/loader.py")
            required.setdefault(
